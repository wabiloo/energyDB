...
function plot_degreedays(buckets, weather, meterType) {
	...
	var data = _.map(years, function(yearbuckets, year) {
		var datax = _.map(yearbuckets, function(bucket) {
			if (bucket.weather) {
				return bucket.weather.MeanDegreeDays;
			}
		});
		var datay = _.map(yearbuckets, function(bucket) {
			if (bucket.weather) {
		...
		return {
			...
			type: 'scatter',
			mode: 'markers',
			name: 'consumption against degree days - year ' + year,
		};
	});
